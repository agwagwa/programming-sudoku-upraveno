Public Class Form1
    ...
    Private Sub Cell_Click(
       ...
       ByVal e As System.EventArgs)
        ...
        ElseIf cellLabel.Text = String.Empty Then
            ...
            '---save the value in the array---
            SetCell(col, row, SelectedNumber, 1)
            DisplayActivity("Number placed at (" & col &
            "," & row & ")", False)
    ...
    Private Sub ToolStripButton_Click(
       ...
       ToolStripButton10.Click
        ...
        Dim selectedButton As ToolStripButton =
    ...
    Private Sub UndoToolStripMenuItem_Click(
       ...
       Handles UndoToolStripMenuItem.Click
        ...
        DisplayActivity("Value removed at (" & Integer.Parse(str(0)) &
    ...
    Private Sub NewToolStripMenuItem_Click(
       ...
       Handles NewToolStripMenuItem.Click
        ...
        ToolStripStatusLabel1.Text = "Generating puzzle..."
            ...
            For col As Integer = 1 To 9
                If actual(col, row) = 0 Then
                    ...
                    If possible(col, row).Length = 1 Then
                        ...
                        DisplayActivity("Inserted value " & actual(col, row) &
                                        " in " & "(" & col & "," & row & ")", False)
    ...
    ' Solve button
    '==================================================
    Private Sub btnSolvePuzzle_Click(
       ByVal sender As System.Object,
       ByVal e As System.EventArgs) _
    ...
    Private Function SolvePuzzle() As Boolean
        ...
        Try
            Do '---Look for Quadruplets in Columns---
                Do '---Look for Quadruplets in Rows---
                    Do '---Look for Quadruples in Minigrids---
                        ...
                        If ExitLoop Then Exit Do
                        '---Look for Quadruples in Minigrids---
                        changes = LookForQuadruplesinMinigrids()
                        If IsPuzzleSolved() Then
                            ExitLoop = True
                            Exit Do
                        End If
                    ...
                    If ExitLoop Then Exit Do
                    '---Look for Quadruplets in Rows---
                    changes = LookForQuadruplesinRows()
                    If IsPuzzleSolved() Then
                        ExitLoop = True
                        Exit Do
                    End If
                ...
                If ExitLoop Then Exit Do
                '---Look for Quadruplets in Columns---
                changes = LookForQuadruplesinColumns()
                If IsPuzzleSolved() Then
                    ExitLoop = True
                    Exit Do
                End If
                        ...
                        For cc As Integer = startC To startC + 2
                            ...
                            If (Not ((cc = c) AndAlso (rr = r))) AndAlso
                               possible(cc, rr) = possible(c, r) Then
                                ...
                                '---twins found---
                                DisplayActivity("Twins found in minigrid at: (" &
                                                c & "," & r & ") and (" &
                                                cc & "," & rr & ")", False)
                ...
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 3 Then
                    ...
                    For rr As Integer = startR To startR + 2
                        For cc As Integer = startC To startC + 2
                            ...
                            If (Not ((cc = c) AndAlso (rr = r))) AndAlso
                               ((possible(cc, rr) = possible(c, r)) OrElse
                                (possible(cc, rr).Length = 2 AndAlso
                                possible(c, r).Contains(
                                possible(cc, rr)(0).ToString()) AndAlso
                                possible(c, r).Contains(
                    ...
                    If tripletsLocation.Length = 6 Then
                        ...
                        '--triplets found---
                        DisplayActivity("Triplets found in " &
            ...
            For r As Integer = 1 To 9
                ...
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 3 Then
                    ...
                    For rr As Integer = 1 To 9
                        If (rr <> r) AndAlso
                           ((possible(c, rr) = possible(c, r)) OrElse
                           (possible(c, rr).Length = 2 AndAlso
                            possible(c, r).Contains(
                            possible(c, rr)(0).ToString()) AndAlso
                            possible(c, r).Contains(
                    ...
                    If tripletsLocation.Length = 6 Then
                        ...
                        '--triplets found---
                        DisplayActivity("Triplets found in " & tripletsLocation,
                           False)

                        '---remove each cell's possible values containing the 
                        ' triplet---
                        For rrr As Integer = 1 To 9
                            If actual(c, rrr) = 0 AndAlso
                               rrr <> CInt(tripletsLocation(1).ToString()) _
                               AndAlso
                               rrr <> CInt(tripletsLocation(3).ToString()) _
                               AndAlso
            ...
            For c As Integer = 1 To 9
                ...
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 3 Then
                    ...
                    For cc As Integer = 1 To 9
                        ...
                        If (cc <> c) AndAlso
                           ((possible(cc, r) = possible(c, r)) OrElse
                            (possible(cc, r).Length = 2 AndAlso
                             possible(c, r).Contains(
                             possible(cc, r)(0).ToString()) AndAlso
                             possible(c, r).Contains(
                        ...
                        For ccc As Integer = 1 To 9
                            If actual(ccc, r) = 0 AndAlso
                               ...
                               ccc <> CInt(tripletsLocation(4).ToString()) Then
                                ...
                                ' values---
                                possible(ccc, r) =
                                   possible(ccc, r).Replace(possible(c, r)(0),
                                   String.Empty)

                                '---remove second triplet number from possible
                                ' values---
                                possible(ccc, r) =
                                   possible(ccc, r).Replace(possible(c, r)(1),
                                   String.Empty)

                                '---remove third triplet number from possible
                                ' values---
                                possible(ccc, r) =
                                   possible(ccc, r).Replace(possible(c, r)(2),
                                   String.Empty)
                                ...
                                If possible(ccc, r).Length = 1 Then
                                    ...
                                    '---saves the move into the stack---
                                    Moves.Push(ccc & r & possible(ccc, r))
                                    DisplayActivity("Look For Triplets in Rows",
                                       False)
                                    DisplayActivity("=========================",
                                       False)
                                    DisplayActivity("Inserted value " &
    ...
    '=========================================================
    'Look for Quadruples in Rows
    '=========================================================
    Public Function LookForQuadruplesinRows() As Boolean
        Dim changes As Boolean = False
        ...
        For r As Integer = 1 To 9
            For c As Integer = 1 To 9
                ...
                '--- four possible values; check for quadruples---
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 4 Then

                    '---first potential quadruple found---
                    '---base quadruplet set---
                    Dim baseSet As String = possible(c, r)

                    '---collect columns that are subset of baseSet---
                    Dim cols As New System.Collections.Generic.List(Of Integer)
                    cols.Add(c)

                    '--scans columns in this row---
                    For cc As Integer = 1 To 9
                        '---look for other quadruples---
                        If cc <> c AndAlso actual(cc, r) = 0 Then
                            Dim candidate As String = possible(cc, r)
                            Dim isSubset As Boolean = True

                            '---every char in candidate must be in baseSet---
                            For i As Integer = 0 To candidate.Length - 1
                                If Not baseSet.Contains(candidate(i)) Then
                                    isSubset = False
                                    Exit For
                                End If
                            Next
                            ...
                            If isSubset Then
                                If Not cols.Contains(cc) Then cols.Add(cc)
                            End If

                            '---if more than 4 cells match, this base can't form a quadruple---
                            If cols.Count > 4 Then Exit For
                        End If
                    Next

                    '--found exactly 4 cells as quadruples; remove all from the other cells---
                    If cols.Count = 4 Then

                        '---build location string for display---
                        Dim quadruplesLocation As String = String.Empty
                        For Each col As Integer In cols
                            quadruplesLocation &= col.ToString() & r.ToString()
                        Next

                        '--quadruples found---
                        DisplayActivity("Quadruples found in " & quadruplesLocation, False)


                        '---remove each cell's possible values containing the
                        ' quadruple---
                        For cccc As Integer = 1 To 9
                            If actual(cccc, r) = 0 AndAlso Not cols.Contains(cccc) Then

                                '---save the original possible values---
                                Dim original_possible As String = possible(cccc, r)

                                '---remove each of the four quadruplet numbers from possible values---
                                For i As Integer = 0 To baseSet.Length - 1
                                    possible(cccc, r) = possible(cccc, r).Replace(baseSet(i), String.Empty)
                                Next

                                '---set the ToolTip---
                                SetToolTip(cccc, r, possible(cccc, r))

                                '---if the possible values are modified, then set 
                                ' the changes variable to True to indicate that the
                                ' possible values of cells in the minigrid have 
                                ' been modified---
                                If original_possible <> possible(cccc, r) Then
                                    changes = True
                                End If
                                ...
                                ' the user has placed a move that results in the 
                                ' puzzle being not solvable---
                                If possible(cccc, r) = String.Empty Then
                                    Throw New Exception("Invalid Move")
                                End If

                                '---if left with 1 possible value for the current 
                                ' cell, cell is confirmed---
                                If possible(cccc, r).Length = 1 Then
                                    SetCell(cccc, r, CInt(possible(cccc, r)), 1)
                                    SetToolTip(cccc, r, possible(cccc, r))

                                    Moves.Push(cccc & r & possible(cccc, r))
                                    DisplayActivity("Look For Quadruples in Rows", False)
                                    DisplayActivity("=========================", False)
                                    DisplayActivity("Inserted value " & actual(cccc, r) & " in " & "(" & cccc & "," & r & ")", False)
    ...
    '=========================================================
    'Look for Quadruples in Columns
    '=========================================================
    Public Function LookForQuadruplesinColumns() As Boolean
        Dim changes As Boolean = False
        ...
        For c As Integer = 1 To 9
            For r As Integer = 1 To 9
                ...
                '--- four possible values; check for tquadruples---
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 4 Then
                    ...
                    '---collect rows that are subset of baseSet---
                    Dim rows As New System.Collections.Generic.List(Of Integer)
                    rows.Add(r)
                    ...
                    For rr As Integer = 1 To 9
                        '---look for other quadruples---
                        If rr <> r AndAlso actual(c, rr) = 0 Then
                            Dim candidate As String = possible(c, rr)
                            Dim isSubset As Boolean = True
                            ...
                            If isSubset Then
                                If Not rows.Contains(rr) Then rows.Add(rr)
                            End If

                            '---if more than 4 cells match, this base can't form a quadruple---
                            If rows.Count > 4 Then Exit For
                        End If
                    Next
                    ...
                    If rows.Count = 4 Then
                        ...
                        For Each row As Integer In rows
                            quadruplesLocation &= row.ToString() & c.ToString()
                        Next
                        ...
                        '---remove each cell's possible values containing the
                        ' quadruple---
                        For rrrr As Integer = 1 To 9
                            If actual(rrrr, c) = 0 AndAlso Not rows.Contains(rrrr) Then

                                '---save the original possible values---
                                Dim original_possible As String = possible(c, rrrr)

                                '---remove each of the four quadruplet numbers from possible values---
                                For i As Integer = 0 To baseSet.Length - 1
                                    possible(c, rrrr) = possible(c, rrrr).Replace(baseSet(i), String.Empty)
                                Next

                                '---set the ToolTip---
                                SetToolTip(c, rrrr, possible(c, rrrr))

                                '---if the possible values are modified, then set 
                                ' the changes variable to True to indicate that 
                                ' the possible values of cells in the minigrid 
                                ' have been modified---
                                If original_possible <> possible(c, rrrr) Then
                                    changes = True
                                End If
                                ...
                                ' the user has placed a move that results in the 
                                ' puzzle beingnot solvable---
                                If possible(c, rrrr) = String.Empty Then
                                    Throw New Exception("Invalid Move")
                                End If

                                '---if left with 1 possible value for the current 
                                ' cell, cell is confirmed---
                                If possible(c, rrrr).Length = 1 Then
                                    SetCell(c, rrrr, CInt(possible(c, rrrr)), 1)
                                    SetToolTip(c, rrrr, possible(c, rrrr))

                                    Moves.Push(c & rrrr & possible(c, rrrr))
                                    DisplayActivity("Look For Quadruples in Columns", False)
                                    DisplayActivity("=========================", False)
                                    DisplayActivity("Inserted value " & actual(c, rrrr) & " in " & "(" & c & "," & rrrr & ")", False)
                                    Application.DoEvents()
    ...
    'Look for Quadruples in Minigrids
    '=========================================================
    Public Function LookForQuadruplesinMinigrids() As Boolean
        Dim changes As Boolean = False

        '---check each cell---
        For r As Integer = 1 To 9
            For c As Integer = 1 To 9
                Dim cells As New List(Of CellPos)
                '--- four possible values; check for quadrupless---
                If actual(c, r) = 0 AndAlso possible(c, r).Length = 4 Then

                    Dim baseSet As String = possible(c, r)

                    cells.Add(New CellPos(c, r))
                    ...
                    For rr As Integer = startR To startR + 2
                        For cc As Integer = startC To startC + 2
                            ...
                            If Not (cc = c AndAlso rr = r) AndAlso actual(cc, rr) = 0 Then

                                Dim candidate As String = possible(cc, rr)
                                Dim isSubset As Boolean = True
                                ...
                                If isSubset Then
                                    cells.Add(New CellPos(cc, rr))
                                End If

                                If cells.Count > 4 Then Exit For
                            End If
                        Next
                    ...
                    If cells.Count = 4 Then
                        ...
                        Dim quadrupleLocation As String = ""
                        For Each cell As CellPos In cells
                            quadrupleLocation &= cell.C.ToString() & cell.R.ToString()
                        Next

                        DisplayActivity("Quadruples found in " & quadrupleLocation, False)

                        For rr As Integer = startR To startR + 2
                            For cc As Integer = startC To startC + 2

                                If actual(cc, rr) = 0 AndAlso Not cells.Contains(New CellPos(cc, rr)) Then

                                    Dim original_possible As String = possible(cc, rr)

                                    For i As Integer = 0 To baseSet.Length - 1
                                        possible(cc, rr) = possible(cc, rr).Replace(baseSet(i), String.Empty)
                                    Next

                                    SetToolTip(cc, rr, possible(cc, rr))

                                    If original_possible <> possible(cc, rr) Then
                                        changes = True
                                    End If

                                    If possible(cc, rr) = "" Then
                                        Throw New Exception("Invalid Move")
                                    End If

                                    If possible(cc, rr).Length = 1 Then
                                        SetCell(cc, rr, CInt(possible(cc, rr)), 1)
                                        SetToolTip(cc, rr, possible(cc, rr))

                                        Moves.Push(cc & rr & possible(cc, rr))
                                        DisplayActivity("Look For Quadruples in Minigrids", False)
                                        DisplayActivity("==============================", False)
                                        DisplayActivity("Inserted value " & actual(cc, rr) &
                                                    " in (" & cc & "," & rr & ")", False)
                                        Application.DoEvents()
    ...
    Public Sub SolvePuzzleByBruteForce()
        ...
        For i As Integer = 0 To possibleValues.Length - 1
            ...
            DisplayActivity("=======================", False)
            DisplayActivity("Trying to insert value " & actual(c, r) &
                            " in " & "(" & c & "," & r & ")", False)
            Try
                If SolvePuzzle() Then
